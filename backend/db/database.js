...
async function updateEvent(id, event) {
    const query = `
        ...
        RETURNING *
    `
    const { rows } = await pool.query(query, [event.status, event.lastUpdatedTimestamp, id])
    return rows[0]
}
...
async function writeEvents(events) {
    ...
    for (const event of events) {
        const existingEvent = await getEventByLocationAndCategory(event.location, event.category)
