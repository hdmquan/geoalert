...
const { parseHtmlData, preprocessData } = await import("../../workers/fetcher.js")
...
test("preprocessData correctly processes event data with geocoding", async () => {
    const geocodeData = JSON.parse(await fs.readFile(path.join(__dirname, "geocode.json"), "utf-8"))
